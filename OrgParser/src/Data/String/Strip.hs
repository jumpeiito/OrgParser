...
testData = do
  ...
  return $ take 20 $ concatMap ((mempty `fromRight`) . orgLineParse) contents

type ThisNode  = OrgNode
type NextNode  = Maybe OrgNode
type ChildNode = Maybe OrgNode
data OrgNode   = OrgNode ThisNode NextNode ChildNode

makeSingleOrgNode :: OrgElement -> OrgNode
makeSingleOrgNode el = OrgNode el Nothing Nothing

addOrgNode :: OrgElement -> OrgNode -> OrgNode
addOrgNode (OrgTitle x) node = undefined
addOrgNode y node = undefined
